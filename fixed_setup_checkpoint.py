...
def call_nemotron(prompt, max_tokens=512):
    ...
    # llama-server/Nemotron may return CoT in reasoning_content instead
    # of literal <think> tags. Normalize so detectors work unchanged.
...
def check_audit_log():
    # Base-OpenClaw version: we are not using NemoClaw/OpenShell audit logs.
    # Use our own local timeline file for reasoning traces, tool calls,
    # flags, and human decisions.
    path = Path.home() / ".openclaw" / "cot_watchdog_timeline.jsonl"
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("a") as f:
        f.write("")
    assert path.exists(), f"could not create timeline at {path}"
    return f"{path} writable"
